...
function persistInterestBasedVariant() {
  const nextContext = createVisitorExperimentContextForVariant("interest_based", readExperimentUserId());
...
function readSavedTopicIds() {
  try {
    ...
    // Storage may be unavailable in embedded test browsers; cookie remains the fallback.
  }

  const cookieTopicId = readCookieValue(COMMUNITY_INTEREST_TOPIC_COOKIE_KEY);
  const decodedTopicId = cookieTopicId ? decodeURIComponent(cookieTopicId) : null;
  return parseCommunityInterestTopicIds(decodedTopicId);
}
...
export function CommunityInterestEntryScreen() {
    ...
    <main className="min-h-screen bg-white text-[#111827]">
      <div className="mobile-shell-wide flex min-h-screen flex-col px-5 pb-6 pt-12">
        ...
        <button
          ...
          type="button"
        >
          추천 피드 보기 ({selectedTopicIds.length}/{COMMUNITY_INTEREST_TOPIC_LIMIT})
