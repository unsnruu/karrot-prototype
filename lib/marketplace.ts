...
export function buildGenericChatPreview(itemId: string, counterpartName = "이웃님", itemTitle?: string): ChatPreview {
  const itemReference = itemTitle ? `${itemTitle}` : "상품";
  const lastMessagePreview = "네, 편하실 때 채팅 주세요. 오늘 저녁 직거래도 괜찮아요.";
  ...
  return {
    ...
    lastMessagePreview,
    messages: [
      { id: `${itemId}-m1`, type: "buyer", text: `안녕하세요! ${itemReference} 아직 가능할까요?`, time: "오후 7:14" },
      { id: `${itemId}-m2`, type: "seller", text: "네, 아직 있어요. 상태 괜찮고 구성도 그대로입니다.", time: "오후 7:17" },
      { id: `${itemId}-m3`, type: "buyer", text: "사진 보고 연락드렸어요. 혹시 금액 5% 정도 조정 가능하실까요?", time: "오후 7:19" },
      { id: `${itemId}-m4`, type: "seller", text: "크게는 어렵지만 내일 합정 쪽에서 바로 거래하시면 조금은 맞춰드릴 수 있어요.", time: "오후 7:23" },
      { id: `${itemId}-m5`, type: "buyer", text: "좋아요. 그럼 내일 퇴근하고 다시 시간 맞춰볼게요!", time: "오후 7:24" },
      { id: `${itemId}-m6`, type: "seller", text: lastMessagePreview, time: "오후 7:26" },
    ],
  };
}
...
export function getChatByItemId(itemId: string) {
  ...
  return buildGenericChatPreview(itemId, seller?.name ?? "이웃님", item?.title);
}
