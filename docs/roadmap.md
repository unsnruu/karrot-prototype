...
다만 전체 로드맵의 최종 도착점은 아래입니다.

- `홈 -> 동네지도` 진입 UI variation 을 만든다.
- variation 을 비교 가능한 형태로 구현한다.
- A/B 테스트로 어떤 UI가 더 높은 진입율을 만드는지 판단한다.
...
목표:
`홈`에서 `동네지도`로 사용자를 이동시키는 진입 포인트를 설계하고, A/B 테스트로 이어질 수 있게 비교 구조를 만듭니다.

범위:
- 홈 안의 후보 진입 포인트 정의
- 기준안과 실험안을 최소 2개 variant 로 정리
- 각 variant 와 동네지도 연결
- variation 간 차이를 설명 가능한 구조로 정리
- 발견 가능성과 전환 행동 비교

기대 결과:
- `무엇을 비교할 것인지`가 명확해집니다.
- A/B 테스트 설계로 넘길 수 있는 UI variation 이 생깁니다.
- 어떤 안이 더 높은 진입율을 만들 가능성이 있는지 해석할 수 있습니다.

### Phase 4. A/B 테스트 준비
목표:
실제 비교 실험으로 넘기기 위한 측정/분기 준비를 정리합니다.

범위:
- variant naming 정리
- 노출/클릭/진입 기준 정의
- 실험 readout 에 필요한 최소 이벤트 관점 정리
- 실험 중 흔들리면 안 되는 고정 맥락 정리

기대 결과:
- UI 비교가 단순 시안 비교가 아니라 실험 가능한 상태가 됩니다.
- 이후 구현, QA, 실험 해석 기준이 같은 문맥을 공유하게 됩니다.
...
- [ ] 첫 번째 `홈 -> 동네지도` 기준안 정의하기
- [ ] 첫 번째 `홈 -> 동네지도` 실험안 정의하기
- [ ] 두 안의 차이가 정확히 무엇인지 문서화하기
- [ ] `동네지도 진입율`을 기준 지표로 명시하기
- [ ] A/B 테스트로 이어질 수 있는 variant 구조 정리하기
