...
- 현재 사용자 흐름을 이해한다.
- `홈` 안에서 `동네지도`로 이어지는 더 나은 UI 진입점을 제안한다.
- 여러 UI 안을 비교 가능한 상태로 만든다.
- 최종적으로 A/B 테스트를 통해 어떤 UI가 `동네지도` 진입율을 더 높이는지 판단한다.
...
`중고거래 홈 흐름 안에서 더 좋은 UI 진입점을 제공하면 동네지도로의 유입을 늘릴 수 있는가?`

그리고 이 질문은 최종적으로 아래 방식으로 판단합니다.

- UI 안을 최소 2개 이상 설계한다.
- 같은 사용자 흐름 안에서 비교 가능하게 구현한다.
- A/B 테스트를 통해 어떤 안이 더 높은 `동네지도` 진입율을 만드는지 확인한다.

실험 설계의 상세 기준은 `docs/experiment-strategy.md`를 참고합니다.
...
- 이번 실험의 핵심은 두 흐름을 더 잘 연결하는 UI를 찾는 것이다.

## 최종 검증 방식
이 프로젝트의 산출물은 단순히 “UI 시안이 있는 프로토타입”에 머물지 않습니다.

최종적으로는 아래 질문에 답할 수 있어야 합니다.

`어떤 UI가 홈에서 동네지도로의 진입율을 가장 더 잘 끌어올리는가?`

그래서 프로토타입은 아래 조건을 만족해야 합니다.

- 비교할 수 있는 UI variation을 담을 수 있어야 한다.
- variation 간 차이가 무엇인지 분명해야 한다.
- 핵심 차이 외의 나머지 맥락은 최대한 동일하게 유지해야 한다.
- 나중에 A/B 테스트 설계로 자연스럽게 이어질 수 있어야 한다.
...
## 한 문장 요약
이 프로젝트는 단순한 당근 클론이 아니라, `홈`에서 `동네지도`로의 진입율을 높이는 UI를 A/B 테스트로 검증하기 위한 읽기 중심 프로토타입입니다.
